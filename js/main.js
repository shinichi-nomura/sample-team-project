"use strict";
{
  ...
  if (logoLink) {
    logoLink.addEventListener("click", (event) => {
      ...
      document.documentElement.style.scrollBehavior = "";
      //URLを#topへ更新する
      history.replaceState(null, "", url.pathname + url.search + "#top");
      window.scrollTo({
        top: 0,
        left: 0,
        behavior: "smooth",
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          requestAnimationFrame(() => {
            ...
            html.classList.remove("initial-anchor-moving");
          });
        });
      });
    };
    ...
    if (targetId === "book") {
      ...
      if (cmsContainers.length > 0) {
        const observer = new MutationObserver(() => {
          ...
          const loadingFinished = cmsContainers.every((container) => {
            return !container.textContent.includes("読み込み中");
          });
          if (loadingFinished) {
            window.setTimeout(() => {
              observer.disconnect();
  ...
  async function initHeroAnimation() {
    const images = gsap.utils.toArray(".hero-images .hero-image-file");
  ...
  function splitTextIntoChars(element) {
    ...
    if (element.dataset.textSplit === "true") {
      return gsap.utils.toArray(element.querySelectorAll(".greeting-char"));
    }
    const textNodes = [];
  ...
  function initSuisoAnimation() {
    ...
    const leadTexts = gsap.utils.toArray(
      suiso.querySelectorAll(".suiso-content > p:not(.suiso-p)"),
    );
    ...
    // h3 → 1つ目のp → 2つ目のpの順番
    const allChars = [...headingChars, ...leadTextChars];
    // 親要素の位置は変更しない
    gsap.set(content, {
      autoAlpha: 1,
    });
  ...
  function initKoseiAnimation() {
    const section = document.querySelector(".koseisinrigaku");
    if (!section) return;
  ...
  function initEventsAnimation() {
    ...
    const panels = gsap.utils.toArray(
      eventsSection.querySelectorAll(".event, .latest"),
    );
  ...
  function initBookAnimation() {
    ...
    const bookItems = gsap.utils.toArray(book.querySelectorAll(".book-item"));
    //「著書」を位置固定で表示
    if (heading) {
      gsap.set(heading, {
        autoAlpha: 0,
  ...
  async function initFooterAnimation() {
    ...
    const footerTopParts = [...footerTopImages, footerTopText].filter(Boolean);
    //footer-top内の画像とテキスト
  ...
  function initFirstGreetAnimation() {
    const section = document.querySelector(".first-greet");
    if (!section) return;
    //二重実行を防止
    if (section.dataset.firstGreetAnimationInitialized === "true") {
      return;
    }
    section.dataset.firstGreetAnimationInitialized = "true";
    const profileImage = section.querySelector(".first-greet-image");
    const profileName = section.querySelector(".name");
    const titles = gsap.utils.toArray(
      section.querySelectorAll(".title-1, .title-2, .title-3, .title-4"),
    );
    const messageFrame = section.querySelector(".message-frame");
    const messages = gsap.utils.toArray(
      section.querySelectorAll(".message-content"),
    );
    const profileParts = [profileImage, profileName].filter(Boolean);
    ...
    section.classList.remove("first-greet-animation-pending");
    //ページ遷移後すぐに動くイントロ
    const introTimeline = gsap.timeline();
    //1. プロフィール画像・名前を同時表示
  ...
  async function initMirrorAnimation() {
    const mirrorSection = document.querySelector(".mirror");
    if (!mirrorSection) return;
    //二重実行を防止
    if (mirrorSection.dataset.mirrorAnimationInitialized === "true") {
      return;
    }
    mirrorSection.dataset.mirrorAnimationInitialized = "true";
    const mirrorUnits = gsap.utils.toArray(
      mirrorSection.querySelectorAll(".left-mirror, .right-mirror"),
    );
    const sliders = gsap.utils.toArray(
      mirrorSection.querySelectorAll(".mirror-slider"),
    );
    const loadingImages = gsap.utils.toArray(
      mirrorSection.querySelectorAll(
        [".mirror-slide", ".l-mirror-frame", ".r-mirror-frame"].join(","),
      ),
    );
    ...
    mirrorSection.classList.remove("mirror-animation-pending");
    //既存の画像切り替えアニメーション
  ...
  function initPegasusAnimation() {
    const section = document.querySelector(".with-pegasus");
    if (!section) return;
    //二重実行を防止
    if (section.dataset.pegasusAnimationInitialized === "true") {
      return;
    }
    section.dataset.pegasusAnimationInitialized = "true";
    const titles = gsap.utils.toArray(
      section.querySelectorAll(
        [".pegasus-title-1", ".pegasus-title-2", ".pegasus-title-3"].join(","),
      ),
    );
    const contents = gsap.utils.toArray(
      section.querySelectorAll(
        [".pegasus-content-1", ".pegasus-content-2"].join(","),
      ),
    );
    ...
    section.classList.remove("pegasus-animation-pending");
    if (typeof ScrollTrigger === "undefined") {
      gsap.set(titles, {
        autoAlpha: 1,
        xPercent: -50,
  ...
  function initContactPageAnimation() {
    ...
    if (!section) return;
    //二重実行を防止
    if (section.dataset.contactPageAnimationInitialized === "true") {
      return;
    }
    section.dataset.contactPageAnimationInitialized = "true";
    ...
    const formUnits = Array.from(contact.children).filter((element) => {
    ...
    const firstVisibleUnits = formUnits.slice(0, 2);
    //それ以降のフォーム
    const remainingUnits = formUnits.slice(2);
    ...
    section.classList.remove("contact-page-animation-pending");
    //ページ表示直後のイントロ
    const introTimeline = gsap.timeline();
    //1. contactの背景・外枠を表示
    ...
    introTimeline.eventCallback("onComplete", () => {
      ...
      ScrollTrigger.batch(remainingUnits, {
        start: "top 88%",
        once: true,
        interval: 0.15,
        batchMax: 3,
  ...
  if (form) {
    ...
    function validateField(field) {
      ...
      if (optionalPhoneIds.has(field.id) && value === "") {
      ...
      if (value === "") {
        showFieldError(
          field,
          emptyMessages[field.id] ?? "この項目を入力してください。",
        );
        return false;
      }
      ...
      if (field.id === "lastName-check" || field.id === "firstName-check") {
        if (!hiraganaPattern.test(value)) {
          showFieldError(field, "ひらがなで入力してください。");
          return false;
        }
      }
      ...
      if (field.id === "email-check") {
        ...
        if (emailInput.validity.valid && value !== originalEmail) {
          showFieldError(
            field,
      ...
      if (optionalPhoneIds.has(field.id)) {
        if (!phonePattern.test(value)) {
          showFieldError(field, "半角数字とハイフンのみで入力してください。");
          return false;
        }
      }
      ...
      if (field.id === "contact-address") {
        ...
        if (postalCode.length !== 7) {
          showFieldError(field, "7桁の郵便番号を入力してください。");
          return false;
        }
      }
      clearFieldError(field);
    ...
    postalCodeInput?.addEventListener("input", () => {
      ...
      if (postalCodeInput.dataset.hasValidated === "true") {
    ...
    addressSearchButton?.addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (!data.results) {
          ...
          showFieldError(postalCodeInput, "住所が見つかりませんでした。");
          return;
        }
        ...
        console.error(error);
        clearSearchedAddress();
        showFieldError(postalCodeInput, "住所検索中にエラーが発生しました。");
      }
    });
  }
}
